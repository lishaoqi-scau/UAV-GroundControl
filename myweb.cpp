...
MyWeb::MyWeb(QWebView * webview, QWidget *parent) :
    QWebPage(parent)
{
    mwebview = webview;
...
void MyWeb::setCoordinate(QString lon,QString lat)
{
...
void MyWeb::on_pushButtonStreetMap_clicked()
{
...
void MyWeb::SetUAVMakerPosition(QString strLon,QString strLat)
{
    QWebFrame *webFrame = mwebview->page()->mainFrame();
...
void MyWeb::on_pushButtonSatelliteMap_clicked()
{
    QWebFrame *frame = mwebview->page()->mainFrame();
...
void MyWeb::getCoordinate(QString lon,QString lat)
{
    emit this->SendLocation(lon,lat);
}


void MyWeb::slotPopulateJavaScriptWindowObject()
{
...
QString MyWeb::userAgentForUrl ( const QUrl & url ) const
{
